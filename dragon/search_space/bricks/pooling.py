...
class AVGPooling3D(nn.Module):
    def __init__(self, pool, p2=None, p3=None, s1=None, s2=None, s3=None):
    ...
    def pad(self,X):
        ...
        sign2 = diff2 / np.abs(diff2) if diff2!=0 else 1
        pad = (int(sign2 * np.ceil(np.abs(diff2)/2)), int(sign2 * np.floor(np.abs(diff2))/2),
               int(sign1 * np.ceil(np.abs(diff1)/2)), int(sign1 * np.floor(np.abs(diff1))/2),
               int(sign0 * np.ceil(np.abs(diff0)/2)), int(sign0 * np.floor(np.abs(diff0))/2))
        return nn.functional.pad(X, pad)
